...
import {IUniswapV3PoolOwnerActions} from "./interfaces/pool/IUniswapV3PoolOwnerActions.sol";
import {IUniswapV3PoolDerivedState} from "./interfaces/pool/IUniswapV3PoolDerivedState.sol";
...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    modifier lock() {
        if (!slot0.unlocked) revert ContractLocked();
    ...
    /// @notice Only the factory owner can call functions with this modifier
    modifier onlyFactoryOwner() {
        require(msg.sender == IUniswapV3Factory(factory).owner(), "Caller is not the factory owner");
        _;
    }
    ...
    function checkTicks(int24 tickLower, int24 tickUpper) private pure {
        ...
        require(tickLower >= TickMath.MIN_TICK, "TickLower must be greater than or equal to MIN_TICK");
        require(tickUpper <= TickMath.MAX_TICK, "TickUpper must be less than or equal to MAX_TICK");
    }
    ...
    /// check
    /// @dev Returns the token0 balance of this contract
    /// @return The token0 balance
    ...
    /// @dev This function is gas optimized to avoid a redundant extcodesize check in addition to the returndatasize
    /// check
    /// @dev Returns the token1 balance of this contract
    /// @return The token1 balance
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        (, int256 amount0Int, int256 amount1Int) = _modifyPosition(
            ModifyPositionParams({
                ...
                tickLower: tickLower,
                tickUpper: tickUpper,
                liquidityDelta: int128(int256(uint256(amount))) // 先转 uint256 → int256 → int128
            })
        );
