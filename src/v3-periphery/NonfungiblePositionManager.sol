    ...
    SelfPermit
{
    ...
    mapping(uint256 => uint256) private _allTokens; // 索引到 tokenId 的映射
    mapping(address => mapping(uint256 => uint256)) private _ownedTokens; // 拥有者到索引到 tokenId 的映射
    mapping(uint256 => uint256) private _ownedTokensIndex; // tokenId 到其在拥有者列表中索引的映射
    mapping(uint256 => uint256) private _allTokensIndex; // tokenId 到其在所有 token 列表中索引的映射
